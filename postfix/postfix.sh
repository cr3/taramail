...
# Remove discontinued DNSBL from existing dns_blocklists.cf
...
if [ ! -z "$DNSBL_CONFIG" ]; then
  ...
  if [ -n "$SPAMHAUS_DQS_KEY" ]; then
    echo -e "\e[32mDetected SPAMHAUS_DQS_KEY variable from mail.conf...\e[0m"
...
# Autogenerated, using Spamhaus DQS reply domains
