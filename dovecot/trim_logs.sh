...
if [[ ! -z ${REDIS_SLAVEOF_IP} ]]; then
  REDIS_CMDLINE="redis-cli -h ${REDIS_SLAVEOF_IP} -p ${REDIS_SLAVEOF_PORT} -a ${REDIS_PASSWORD} --no-auth-warning"
else
  REDIS_CMDLINE="redis-cli -h taramail-redis -p 6379 -a ${REDIS_PASSWORD} --no-auth-warning"
fi
